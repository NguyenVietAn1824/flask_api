...
def create_cat():
    ...
    cat_id = 'cat' + str(len(CATS) + 1)

    # TODO 2: Lấy dữ liệu từ request
    new_cat = {
        'id': cat_id,  # Lấy ID của mèo từ biến cat_id
        'name': request.json['name'],  # Lấy tên của mèo từ dữ liệu JSON trong yêu cầu
        'age': request.json.get('age', 0),  # Lấy tuổi của mèo, nếu không có thì mặc định là 0
        'image_link': request.json.get('image_link', None)  # Lấy đường dẫn hình ảnh, nếu không có thì mặc định là None
    }
